class_name WDB_NodeFunction extends WDB_NodeContainer

var objectData : WDB_NodeFunctionData

func LoadFromBuffer(file : StreamPeerBuffer, loader : WDB_FileLoader) -> void:
	self.name = "NodeFunction_%s" % self.offset_hex
	tag = 120
	
	#импорт NodeRootData
	file.seek(file.get_position() + 8) #skip tag (1) and size
	objectData = WDB_NodeFunctionData.new()
