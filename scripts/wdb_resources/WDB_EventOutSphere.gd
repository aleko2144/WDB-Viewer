class_name WDB_EventOutSphere extends WDB_EventSimple

var eventData : WDB_EventOutSphereData

func LoadFromBuffer(file : StreamPeerBuffer, loader : WDB_FileLoader) -> void:
	self.name = "EventOutSphere_%s" % self.offset_hex
	tag = 132
	
	#импорт данных
	file.seek(file.get_position() + 8) #skip tag (1) and size
	eventData = WDB_EventOutSphereData.new()
