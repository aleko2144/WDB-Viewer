class_name WDB_EventInCylinder extends WDB_EventSimple

var eventData : WDB_EventInCylinderData

func LoadFromBuffer(file : StreamPeerBuffer, loader : WDB_FileLoader) -> void:
	self.name = "EventInCylinder_%s" % self.offset_hex
	tag = 129
	
	#импорт данных
	file.seek(file.get_position() + 8) #skip tag (1) and size
	eventData = WDB_EventInCylinderData.new()
