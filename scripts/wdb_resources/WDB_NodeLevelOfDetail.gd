class_name WDB_NodeLevelOfDetail extends WDB_NodeContainer

var objectData : WDB_NodeLevelOfDetailData

func LoadFromBuffer(file : StreamPeerBuffer, loader : WDB_FileLoader) -> void:
	self.name = "NodeLevelOfDetail_%s" % self.offset_hex
	tag = 121
	
	#импорт NodeRootData
	file.seek(file.get_position() + 8) #skip tag (1) and size
	objectData = WDB_NodeLevelOfDetailData.new()
